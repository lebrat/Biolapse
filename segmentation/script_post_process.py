...
## Load net
path_h5 = '../Data/Segmentation/Model/'+name+'.h5'
...
if model_name =='Unet3D':
    ...
    if SAVE:
        y_pred = visualize.save_png_list(net, X_test, name=name, thresh=thresh, format_im=type_im) 
    # Display an image
    ...
    if SAVE:
        y_pred = visualize.save_png_list_channel_first(net, X_test, name=name, thresh=thresh, format_im=type_im)
else:
...
if VIDEO and not(model_name=='Unet2D'):
    visualize.save_png_video(net, X_test, thresh=thresh, format_im=type_im,name=name+'_demo_video')
    visualize.automatic_process(name+'_demo_video',cut=10,nb_movie=2,nb_seq_in_video=130)

## Compute score on test
t_pred = time.time()
y_pred = np.array(net.predict(X_test)>thresh,np.uint8).flatten()
