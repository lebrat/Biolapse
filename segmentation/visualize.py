...
def display_temporal_im(im,nb_time_serie=1):
	for k in range(nb_time_serie):
		for t in range(im.shape[3]):
			pylab.figure()
			pylab.imshow(im[k,:,:,t])
			pylab.title('Figure {0}, time {1}'.format(k,t))

def close_all():
	pylab.close('all')

def display_loss(name,save=False):
	tmp=pickle.load(open(os.getcwd()+'/Data/Weights/'+name+'plots.p','rb'))
	plt.figure()
	plt.plot(tmp['loss'][2:])
	...
	if save:
		plt.savefig(os.getcwd()+'/Data/'+name+'_loss.png')
	...
	# plt.legend(['train', 'test'], loc='upper left')
	# if save:
	# 	plt.savefig(os.getcwd()+'/Data/'+name+'_acc.png')
...
"""
def display_images_temps(model,imgs,N=2,tresh=0.):
	leng=len(imgs)
	...
	for i in ite:
		...
		plt.figure(6)
		plt.imshow(np.array(out>tresh,dtype=float))
		plt.title('Prediction tresholded')
		plt.show()
...
"""
def save_png_list(model, imgs, name='default', tresh=0, format_im=np.uint8):
	leng=len(imgs)
	t=1
	cpt=0
	pred = []
	for i in range(imgs.shape[0]):
		img=imgs[i]
		if tresh > 0:
			out = np.array(np.squeeze(model.predict(np.expand_dims(img,0)))>tresh,dtype=np.float)
		else:
...
Save images in two different names.
"""
def save_png_list_channel_first(model, imgs, name='default', tresh=0, format_im=np.uint8):
	leng=len(imgs)
	cpt=0
	pred = []
	for i in range(imgs.shape[0]):
...
"""
Save images follow by mask to make merged video
"""
def save_png_video(model, imgs, name='default', tresh=0, format_im=np.uint8,post_process=True):
	leng=len(imgs)
	cpt=0
	for i in range(imgs.shape[0]):
		img=imgs[i]
...
"""
def post_process_temporal(path_imgs, kernel_radius=3, start_code='mask_', compute_imgs=False, model='', name='default', format_im=np.uint8, nx=256, ny=256, TIME=10, tresh=0.5):
	if compute_imgs or model=='':
		...
		save_png_list(model, X, name=name, tresh=0, format_im=np.uint8)
				...
				for i in range(len(masks)):
					if i==0:
						...
						for j in range(1,ncomponents+1):
							mask_tmp=np.array(map_class==j,dtype='f')
							if np.sum(mask_tmp)>tresh2: # Si trop petit on le regarde meme pas
...
"""
Automatically take the output of save_png_video, remove first 'cut' frames, make 'nb_serie' movies.
"""
def automatic_process(name):
	...
	cut=10
	nb_serie=2
	nb_in_serie=130 # nb of images per movie
	tmp = os.path.join('Outputs',name,'tmp','*.png')
	import shutil
	for i in range(nb_serie):
		for im in glob.glob(tmp)[i*(nb_in_serie-cut):i*(nb_in_serie-cut)+cut]:
			os.remove(im)
		tmp2 = os.path.join('Outputs',name,'tmp','film{0}'.format(i))
		if not os.path.exists(tmp2):
			os.makedirs(tmp2)
		for im in glob.glob(tmp)[i*(nb_in_serie-cut):(i+1)*(nb_in_serie-cut)]:
...
def main():
	...
	save_png_list(model, X, tresh=0.2, format_im=format_im,name=name)
...
# name='NN_Unet3D_filt4_depth4_Thomas'
# path_h5 = 'Data/Model_h5/'+name+'.h5'
# X = generator_temps_no_mask(path_imgs,nx=nx,ny=ny,TIME = TIME, format_im=format_im)
# X = np.expand_dims(np.array(X,dtype=np.float)/np.iinfo(format_im).max,4)
# model = load_model(path_h5)
# save_png_video(model, X, tresh=0, format_im=format_im,name=name)
# automatic_process(name)
# tresh=0.1
# X = generator_temps_no_mask(path_imgs,nx=nx,ny=ny,TIME = TIME, format_im=format_im)
# X = np.expand_dims(np.array(X,dtype=np.float)/np.iinfo(format_im).max,4)
# save_png_video(model, X, tresh=tresh, format_im=format_im,name=name+'tresh{0}'.format(tresh))
# automatic_process(name+'tresh{0}'.format(tresh))
# tresh = 0.2
# save_png_video(model, X, tresh=tresh, format_im=format_im,name=name+'tresh{0}'.format(tresh))
# automatic_process(name+'tresh{0}'.format(tresh))
# tresh = 0.8
