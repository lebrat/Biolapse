...
"""
def tiff_to_png(path_tiff,path_png,nx=256,ny=256,type_im=np.uint8,sample_rate=1,t_end_=1e8):
...
"""
Run through path and aggregate time images into batch of specify size.
Return two matching dataset specify by   
"""
def path_to_time_batchs(path_fold,nx=128,ny=128,TIME=15,type_im=np.uint8,format_im='png',code_im1='images',code_im2='masks'):
    ...
    for (fold, subfold, files) in os.walk(path_fold):
        if os.path.basename(fold)==code_im1:
            nb_serie = int(len([f for f in files if f.endswith(format_im)])/TIME)
            images_path = sorted([os.path.join(fold,f) for f in files if f.endswith(format_im)], key = utils.stringSplitByNumbers)
            cpt =0
            for i in range(nb_serie):
                for t in range(TIME):
                    ...
                    img = resize(img, (nx, ny), mode='constant', preserve_range=True)
                    X_train[cpt_serie,:,:,t]=img
                    mask = imageio.imread(images_path[cpt].replace(code_im1,code_im2))
                    mask = resize(mask, (nx, ny), mode='constant', preserve_range=True)
                    mask = np.array(np.iinfo(type_im).max*(mask>0.01), dtype=type_im)
...
def array_to_npy(x,y,test=False,name='default'):
    ...
    for c in range(x.shape[0]):
        if test:
            np.save(os.path.join(os.getcwd(),'tmp',name,'images',str(99999999+c)+'.npy'),x[c])
            np.save(os.path.join(os.getcwd(),'tmp',name,'masks',str(99999999+c)+'.npy'),y[c])
            List_id.append(99999999+c)
        else:
            np.save(os.path.join(os.getcwd(),'tmp',name,'images',str(c)+'.npy'),x[c])
            np.save(os.path.join(os.getcwd(),'tmp',name,'masks',str(c)+'.npy'),y[c])
            List_id.append(c)
    return List_id
...
class DataGenerator3D(keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            # Store sample
            X[i] = np.load(os.getcwd()+'/tmp/'+self.name+'/images/' + str(ID) + '.npy')
            # Store class
            Y[i] = np.load(os.getcwd()+'/tmp/'+self.name+'/masks/' + str(ID) + '.npy')
        return X, Y
 ...
 rotation1=30, rotation2=150,rotation3=240,rotation4=90,rotation5=290,rotation6=120, rotation7=330):
	...
	if original:
		...
		for k in range(len(true_imgs)):
			...
			imageio.imsave(os.path.join(path_save,'Original','masks',str(k).zfill(10)+'.png'),im)
			im_true = imageio.imread(true_imgs[k])
			imageio.imsave(os.path.join(path_save,'Original','images',str(k).zfill(10)+'.png'),im_true)
	...
	if oneOfTwo:
		...
		for k in range(len(true_imgs)):
			if k%2==0:
				im = imageio.imread(true_masks[k])
				imageio.imsave(os.path.join(path_save,'OneOfTwo','masks',str(k).zfill(10)+'.png'),im)
				im_true = imageio.imread(true_imgs[k])
				imageio.imsave(os.path.join(path_save,'OneOfTwo','images',str(k).zfill(10)+'.png'),im_true)
	...
	if reverse:
		...
		for k in range(len(true_imgs)):
			...
			imageio.imsave(os.path.join(path_save,'Reverse','masks',str(k).zfill(10)+'.png'),im)
			im_true = imageio.imread(true_imgs[k_max-1-k])
			imageio.imsave(os.path.join(path_save,'Reverse','images',str(k).zfill(10)+'.png'),im_true)
	...
	if reverseOneOfTwo:
		...
		for k in range(len(true_imgs)):
			if k%2==0:
				im = imageio.imread(true_masks[k_max-1-k])
				imageio.imsave(os.path.join(path_save,'ReverseOneOfTwo','masks',str(k).zfill(10)+'.png'),im)
				im_true = imageio.imread(true_imgs[k_max-1-k])
				imageio.imsave(os.path.join(path_save,'ReverseOneOfTwo','images',str(k).zfill(10)+'.png'),im_true)
	...
	for k in range(len(true_imgs)):
		...
		if rotation1!=0:
			...
			imageio.imsave(os.path.join(path_save,'Rotation1','masks',str(k).zfill(10)+'.png'),np.array(tmp_im))
			tmp_im=im_true_.rotate(rotation1)
			imageio.imsave(os.path.join(path_save,'Rotation1','images',str(k).zfill(10)+'.png'),np.array(tmp_im))

		if rotation2!=0:
			tmp_im=im_.rotate(rotation2)
			imageio.imsave(os.path.join(path_save,'Rotation2','masks',str(k).zfill(10)+'.png'),np.array(tmp_im))
			tmp_im=im_true_.rotate(rotation2)
			imageio.imsave(os.path.join(path_save,'Rotation2','images',str(k).zfill(10)+'.png'),np.array(tmp_im))
		
		if rotation3!=0:
			tmp_im=im_.rotate(rotation3)
			imageio.imsave(os.path.join(path_save,'Rotation3','masks',str(k).zfill(10)+'.png'),np.array(tmp_im))
			tmp_im=im_true_.rotate(rotation3)
			imageio.imsave(os.path.join(path_save,'Rotation3','images',str(k).zfill(10)+'.png'),np.array(tmp_im))

		if k%2==0:
			if rotation4!=0:
				tmp_im=im_.rotate(rotation4)
				imageio.imsave(os.path.join(path_save,'Rotation4','masks',str(k).zfill(10)+'.png'),np.array(tmp_im))
				tmp_im=im_true_.rotate(rotation4)
				imageio.imsave(os.path.join(path_save,'Rotation4','images',str(k).zfill(10)+'.png'),np.array(tmp_im))

			if rotation5!=0:
				tmp_im=im_.rotate(rotation5)
				imageio.imsave(os.path.join(path_save,'Rotation5','masks',str(k).zfill(10)+'.png'),np.array(tmp_im))
				tmp_im=im_true_.rotate(rotation5)
				imageio.imsave(os.path.join(path_save,'Rotation5','images',str(k).zfill(10)+'.png'),np.array(tmp_im))
		...
		if rotation6!=0:
			...
			imageio.imsave(os.path.join(path_save,'Rotation6','masks',str(k).zfill(10)+'.png'),np.array(tmp_im))
			tmp_im=im_true_.rotate(rotation6)
			imageio.imsave(os.path.join(path_save,'Rotation6','images',str(k).zfill(10)+'.png'),np.array(tmp_im))

		if rotation7!=0:
			tmp_im=im_.rotate(rotation7)
			imageio.imsave(os.path.join(path_save,'Rotation7','masks',str(k).zfill(10)+'.png'),np.array(tmp_im))
			tmp_im=im_true_.rotate(rotation7)
			imageio.imsave(os.path.join(path_save,'Rotation7','images',str(k).zfill(10)+'.png'),np.array(tmp_im))
